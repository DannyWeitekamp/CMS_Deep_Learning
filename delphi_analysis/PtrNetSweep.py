...
from keras.layers import Dense, Dropout, merge, Input, LSTM, Masking,GRU,RepeatVector,Merge
...
def build_LSTM_model(name, input_width,out_width, depth,recurrent_width, lstm_activation="tanh", lstm_dropout = 0.0,
    ...
    inputs = []
    x = a = Input(shape=(SEQ_LEN , input_width), name="input")
    inputs.append(a)
    ...
    p  = Ptr_Layer(16,sparsity_coeff=0.0,implementation='ptr_net',seq_len=SEQ_LEN)([x,e,d])
    a = p
...
if __name__ == '__main__':
    ...
    for trial in trials:
        print(trial.summary())
        trial.execute(custom_objects={"Slice":Slice, "Ptr_Layer":Ptr_Layer})
