...
if __package__ is None:
    sys.path.append(os.path.realpath("/bigdata/shared/Software/Attention"))
    sys.path.append(os.path.realpath("/bigdata/shared/Software/AttentionWithContext"))
...
def build_LSTM_model(name, input_width,out_width, depth, lstm_activation="tanh", lstm_dropout = 0.0,
                     dropout=0.0, output_activation="softmax", single_list=False,attention=None, **kargs):
    inputs = []
    ...
    for i in range(depth):
        ...
        a = GRU(input_width,
                 ...
                 name = "gru_" +str(i),
                 return_sequences = attention != None
                 )(a)
        if(attention == "Attention"):
            a = Attention(bias=False)(a)
        elif(attention == "AttentionWithContext"):
            a = AttentionWithContext(bias=False)(a)
        if(dropout > 0.0):
...
def build_trial(name,
    ...
    if isinstance(model, types.FunctionType):
        model = model(**kargs)
    if (workers == 1):
        trial = KerasTrial(archive_dir, name=name, model=model, seed=0,custom_objects={"Attention": Attention, "AttentionWithContext": AttentionWithContext})
        ...
        trial = MPI_KerasTrial(archive_dir, name=name, model=model, workers=workers, seed=0, features_name="Particles",labels_name="Labels",custom_objects={"Attention": Attention, "AttentionWithContext": AttentionWithContext})
            ...
            for depth in [1]:
                ...
                for activation in ['tanh']:
                    ...
                    for lstm_dropout in [0.0]:
                        for attention in ["Attention","AttentionWithContext"]:
                                ...
                                for train in KFold(g[:ntf], int(K_items), 40):
                                    ...
                                    inps = f(name='GRU_' + attention, 
                                         ...
                                         keys_to_record=['labels', 'depth', 'sort_on', 'sort_ascending',
                                                         ...
                                                         'patience', "n_train_files",'attention'],
                                         sort_on=sort_on,
...
if __name__ == '__main__':
    ...
    for trial in trials:
        print(trial.summary())
        trial.execute(custom_objects={"Attention": Attention, "AttentionWithContext": AttentionWithContext})
