            ...
            if (isinstance(leaf, DataProcessingProcedure)):
                ...
                for l in proc.input_leaves:
                    ...
                    if (isinstance(obj, ROOT.TLeafElement) == False):
                        raise ValueError(
                            "Input Leaf %r in Procedure %r does not exist in Tree %r" % (l, str(proc), tree_name))
        ...
        for entry in range(n_entries):
            if (verbosity > 0):
                ...
                if (c > last_time + .25):
                    ...
                    sys.stdout.write("[%-20s] %r/%r  %r(Entry/sec)" % (
                        '=' * int(20 * percent), entry, int(n_entries), 4 * (entry - prev_entry)))
                    sys.stdout.flush()
                    last_time = c
                    prev_entry = entry
...
def getPandasNumValues(filename):
    return ROOT_NumValues_to_pandas(filename,
                                    ["Photon.Phi", "Electron.Phi", "MuonTight.Phi", "MissingET.Phi", "EFlowPhoton.Phi",
                                     "EFlowNeutralHadron.Phi", "EFlowTrack.Phi"],
                                    columns=["Photon", "Electron", "MuonTight", "MissingET", "EFlowPhoton",
                                             "EFlowNeutralHadron", "EFlowTrack"])
